...
static class XPManager
{
    public static int CalculateXP(Enemy e)
    {
        ...
        int grayLevel = CalculateGreyLevel();

        int totalXP = 0;

        if(e.MyLevel >= Player.MyInstance.MyLevel)
        {
            totalXP = (int)(baseXP * (1 + 0.5 * (e.MyLevel - Player.MyInstance.MyLevel)));
        }

        if (e.MyLevel > grayLevel)
        {
    ...
    public static int CalculateGreyLevel()
    {
